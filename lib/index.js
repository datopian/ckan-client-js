...
class DataHub {
  ...
  async push(file, token) {
    ...
    const verifyAction = await CkanUploadAPI.validateActionsBeforeUpload(lfs)

    // Upload the file to cloud storage
    const cloudStorage = await CkanUploadAPI.uploadToStorage(lfs.objects[0].actions.upload, file.content())

    // If cloud storage return 201(OK), Get request to verify signed URL.
    const verifyUpload = await CkanUploadAPI.verifyUpload(lfs.objects[0].actions.verify, file)
