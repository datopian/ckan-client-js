...
class GitLfsMultipartTransfer {
  ...
  async _readChunk(start, size) {
    // TODO: read `size` bytes from file starting from pos `start`
    ...
    // But since parts are not likely to be extremely large (likely size is up
    // to a few 10s of MBs), it would be OK to read the entire part into memory
    // as well, and may simplify things esp as we may also need to get a digest
    // of the same part content
  }
